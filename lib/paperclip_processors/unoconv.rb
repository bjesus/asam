  ...
  class Unoconv < Processor
    ...
    def make
      ...
      begin
        ...
        #success = Paperclip.run("office2jpg", parameters, :source => "#{File.expand_path(src.path)}#{'[0]' unless animated?}", :dest => File.expand_path(dst.path))
        puts "office2jpg", parameters, :source => "#{File.expand_path(src.path)}#{'[0]' unless animated?}", :dest => File.expand_path(dst.path)
